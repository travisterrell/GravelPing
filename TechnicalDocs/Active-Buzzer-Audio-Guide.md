...
The GravelPingRX-S3 uses an **active buzzer** on GPIO6 for audio backup alerts. An active buzzer has a built-in oscillator and produces a fixed-frequency tone when powered - no PWM frequency control needed, just simple on/off.

This is much simpler and louder than the passive piezo/PWM approach, making it ideal for outdoor alerts.

## Hardware Setup

### What is an Active Buzzer?

**Active Buzzer (what we're using):**
- Has built-in oscillator circuit
- Just needs power (GPIO HIGH = on, LOW = off)
- Produces fixed tone (typically 2-4 kHz)
- Very loud (85-95 dB typical)
- Simple 2-wire connection

**Passive Piezo (NOT using):**
- Requires PWM at specific frequencies
- More flexible (any tone) but quieter
- Needs amplifier for outdoor use
...
**Option 1: Direct Connection (Most Common)**
```
...
**Option 2: Transistor Driver (High Current)**
```
ESP32-S3 5V         ──► Buzzer (+)
...
If reversed, buzzer won't work (but won't damage anything - just swap the wires).
